...
Después de revisar los programas, considero que Python tiene la sintaxis más fácil de comprender. Su estructura es muy simple, no necesita declarar tipos de datos ni usar muchas palabras clave, y las operaciones se expresan de forma directa.
...
**En Scala (forma funcional))**
```scala
def getAverage(nums: List[Int]): Double =
  nums.sum.toDouble / nums.size
```
**En Python (forma funcional))**
```python
def get_average(nums):
    return sum(nums) / len(nums)
```
**En Haskell (forma funcional))**
```haskell
